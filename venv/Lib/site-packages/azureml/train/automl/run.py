...
from azureml.automl.core.shared._diagnostics.automl_error_definitions import (
    ...
    ContinueRunUnsupportedForUntrackedRuns, DependencyWrongVersion, FeatureUnsupportedForIncompatibleArguments,
    InvalidArgumentType, InvalidIteration, InvalidOperationOnRunState, ModelMissing, NoMetricsData,
    LoadModelDependencyMissing, OnnxNotEnabled, OnnxSplitsNotEnabled, RuntimeModuleDependencyMissing)
...
class AutoMLRun(Run):
    ...
    def _get_onnx_run(self,
        ...
        Contract.assert_value(curr_run, "current_run")
        ...
        with log_server.new_log_context():
            ...
            with logging_utilities.log_activity(
                ...
                if return_onnx_model:
                    fitted_model = self._download_automl_onnx_model(curr_run, model_name)
                else:
                    fitted_model = self._download_automl_model(curr_run, model_name)
    ...
    def _download_automl_model(self, curr_run: Run, model_name: str = constants.MODEL_PATH) -> Optional[Any]:
        with logging_utilities.log_activity(
            logger,
            ...
            model_path = 'model_{}.pkl'.format(curr_run.id)
            ...
            try:
                ...
                model = self._load_automl_model(model_path)
                if model is None:
                    ...
                    automl_runtime_ver = run_deps.get("azureml-train-automl-runtime", None)
    ...
    def _load_automl_model(self, model_path: str) -> Optional[Any]:
        """
    ...
    def _download_automl_onnx_model(self, curr_run: Run, model_name: str) -> Any:
        model_path = 'model_onnx_{}.onnx'.format(curr_run.id)
    ...
    def _fail_with_error(self, exception: BaseException) -> None:
        ...
        interpreted_exception = common_utilities.interpret_exception(exception, self._is_local())
        ...
        # Note that exceptions coming from the networking layer won't have error responses on them.
        if isinstance(interpreted_exception, AzureMLServiceException) and hasattr(interpreted_exception, "error"):
            # Extract the ErrorResponse from the exception sent by the service
            error_response = json.loads(str(interpreted_exception.error))
            ...
            super(AutoMLRun, self).fail(error_details=interpreted_exception,
