...
def get_docker_port(docker_client, container_name, container):
    ...
    for key, value in container.attrs['NetworkSettings']['Ports'].items():
        if str(DOCKER_IMAGE_HTTP_PORT) in key and value:
...
def container_scoring_call(docker_port, input_data, container, health_url, cleanup_on_failure=True):
    ...
    :rtype: str
    """
    scoring_url = health_url + WEBSERVICE_SCORE_PATH
...
def convert_parts_to_environment(service_name, inference_config):
    ...
    if not inference_config.environment and inference_config.runtime == 'python' \
            ...
            and not (inference_config.base_image and (inference_config.enable_gpu or
                                                      inference_config.cuda_version)):
