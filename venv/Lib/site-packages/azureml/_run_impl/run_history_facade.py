...
class RunHistoryFacade(ChainedIdentity):
    ...
    def get_metrics(self, name=None, recursive=False, run_type=None, populate=False, root_run_id=None, run_ids=None,
        ...
        if _use_v2_metrics and name is not None and run_type is None:
            return_value = {}
            named_metric = self.metrics.get_metric_for_run_v2(name, self.artifacts, populate)
            if named_metric is not None:
                return_value[name] = named_metric
            return return_value
    ...
    def _error_details_to_dictionary(error_details=None, error_code=None):
        ...
        if isinstance(error_details, six.text_type):
            ...
            if isinstance(error_details, AzureMLException) and \
                    hasattr(error_details, '_azureml_error') and \
                    error_details._azureml_error is not None:
                error_dict = error_details._azureml_error.to_root_error_dict()
            else:
